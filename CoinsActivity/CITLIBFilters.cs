...
namespace ImageProcess2
{
    ...
    {
        // Coin values
        public const float FivePeso = 5f;
        public const float OnePeso = 1f;
        public const float TwentyFiveC = 0.25f;
        public const float TenC = 0.10f;
        public const float FiveC = 0.05f;

        public static void AnalyzeCoins(Bitmap image, ref Label totalValueLabel)
        {
            float totalValue = 0;
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    if (!visited[y, x] && IsCoin(image.GetPixel(x, y)))
                    {
                        int coinPixelCount = BFS(image, ref visited, x, y);
                        float coinValue = IdentifyCoin(coinPixelCount, ref fivePesoCount, ref onePesoCount, ref twentyFiveCCount, ref tenCCount, ref fiveCCount);
            ...
            string logFilePath = "coins_acitivity_log.txt";
            File.AppendAllText(logFilePath, $"5 Peso Coins: {fivePesoCount}\n"); // Should be 5
            File.AppendAllText(logFilePath, $"1 Peso Coins: {onePesoCount}\n"); // Should be 13
            File.AppendAllText(logFilePath, $"25 Cent Coins: {twentyFiveCCount}\n"); // Should be 28
            File.AppendAllText(logFilePath, $"10 Cent Coins: {tenCCount}\n"); // Should be 11
            File.AppendAllText(logFilePath, $"5 Cent Coins: {fiveCCount}\n"); // Should be 7
        }

        private static float IdentifyCoin(int pixelCount, ref int fivePesoCount, ref int onePesoCount, ref int twentyFiveCCount, ref int tenCCount, ref int fiveCCount)
        {
			if (pixelCount >= 17500)
			{
				fivePesoCount++;
        ...
        private static int BFS(Bitmap image, ref bool[,] visited, int startX, int startY)
        {
            int height = image.Height;
            int width = image.Width;
            int pixelCount = 0;
            ...
            while (queue.Count > 0)
            {
                ...
                AddPixel(image, visited, queue, x - 1, y);
                AddPixel(image, visited, queue, x + 1, y);
                AddPixel(image, visited, queue, x, y - 1);
                AddPixel(image, visited, queue, x, y + 1);
            }
        ...
        private static void AddPixel(Bitmap image, bool[,] visited, Queue<Point> queue, int x, int y)
        {
            int height = image.Height;
            int width = image.Width;

            if (x >= 0 && x < width && y >= 0 && y < height && !visited[y, x] && IsCoin(image.GetPixel(x, y)))
            {
                queue.Enqueue(new Point(x, y));
            }
        }

        private static bool IsCoin(Color pixel)
        {
